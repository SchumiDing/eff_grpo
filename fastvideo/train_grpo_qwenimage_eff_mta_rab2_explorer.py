...
#
# RAB2-Explorer: max-quality-oriented Residual-AB2 fork.
#
# Same guess/infer schedule and late-phase velocity AB2 as RAB2. Early group-fusion
# still builds residual_prev / residual_pp / residual_so like RAB2, but within each
# prompt group we rank ||v_prev - v_prev_group|| among guessed rollouts only and map
# rank to per-rollout alpha_floor and residual growth (anchors conservative, explorers
# aggressive). Deterministic tie-break; no extra random noise.
...
# Best-so-far base is early_frac=0.4 with stronger AB2. Residual-AB2 keeps that compute
# schedule but extrapolates rollout-specific residuals around the prompt group mean.
_MTA_RAB2_EARLY_FRAC = 0.4
_MTA_RAB2_C1 = 2.0
_MTA_RAB2_C2 = -1.0
_MTA_RAB2_RESIDUAL_BIAS = 0.25
_MTA_RAB2_RESIDUAL_GROWTH = 1.35

_MTA_RAB2_EXPLORER_ALPHA_FLOOR_LO = 0.18
_MTA_RAB2_EXPLORER_ALPHA_FLOOR_HI = 0.42
_MTA_RAB2_EXPLORER_GROWTH_LO = 1.20
_MTA_RAB2_EXPLORER_GROWTH_HI = 1.55
...
def _clip_like_reference(
    value: torch.Tensor,
    reference: torch.Tensor,
    growth: float,
    eps: float = 1e-8,
) -> torch.Tensor:
    value_norm = _row_l2_norm(value, eps=eps)
    ref_norm = _row_l2_norm(reference, eps=eps)
    max_norm = growth * ref_norm
    scale = (max_norm / value_norm).clamp(max=1.0)
    return value * scale.view(-1, *([1] * (value.ndim - 1)))


def _clip_like_reference_per_row_growth(
    value: torch.Tensor,
    reference: torch.Tensor,
    growth: torch.Tensor,
    eps: float = 1e-8,
...
def _explore_strength_from_residual_energy_per_prompt(
    guessed_mask: torch.Tensor,
    old_prev: torch.Tensor,
    old_group: torch.Tensor,
    num_prompts: int,
    num_generations: int,
) -> torch.Tensor:
    """0=anchor (lowest ||v_prev-v_group|| among guessed in group), 1=explorer."""
    B = guessed_mask.shape[0]
    device = guessed_mask.device
    out = torch.zeros(B, device=device, dtype=torch.float32)
    if not guessed_mask.any():
        return out
    residual_prev_all = old_prev.to(torch.float32) - old_group.to(torch.float32)
    for p in range(num_prompts):
        start = p * num_generations
        end = (p + 1) * num_generations
        gslice = guessed_mask[start:end]
        if not gslice.any():
            continue
        local = torch.where(gslice)[0] + start
        n = int(local.numel())
        energy = _row_l2_norm(residual_prev_all[local])
        if n == 1:
            out[local] = 0.5
            continue
        idx = torch.arange(n, device=device, dtype=energy.dtype)
        emax = energy.max().abs()
        tie_break = idx * (emax + 1.0) * 1e-12
        order = torch.argsort(energy + tie_break)
        ranks = torch.zeros(n, device=device, dtype=torch.long)
        ranks[order] = torch.arange(n, device=device, dtype=torch.long)
        s = ranks.to(torch.float32) / float(n - 1)
        out[local] = s
    return out
...
def run_sample_step(
    ...
    if grpo_sample:
        ...
        prev_group_velocities = torch.zeros_like(z, dtype=torch.float32)
        prev_prev_group_velocities = torch.zeros_like(z, dtype=torch.float32)
        ...
        for i in progress_bar:
            ...
            old_group = prev_group_velocities.clone()
            old_ppgroup = prev_prev_group_velocities.clone()
            ...
            # 预测分支：前半段对 rollout 残差做 AB2 外推，后半段退化到完整速度 AB2。
            early_group_fusion = i < len(progress_bar) * _MTA_RAB2_EARLY_FRAC
            if guessed_mask.any():
                ...
                if early_group_fusion:
                    ...
                    residual_prev = v_prev - old_group[guessed_mask]
                    residual_pp = v_pp - old_ppgroup[guessed_mask]
                    if i >= 2:
                        residual_so = _MTA_RAB2_C1 * residual_prev + _MTA_RAB2_C2 * residual_pp
                    else:
                        residual_so = residual_prev
                    s_rows = _explore_strength_from_residual_energy_per_prompt(
                        guessed_mask,
                        old_prev,
                        old_group,
                        num_prompts,
                    ...
                    growth_row = (
                        _MTA_RAB2_EXPLORER_GROWTH_LO
                        + (_MTA_RAB2_EXPLORER_GROWTH_HI - _MTA_RAB2_EXPLORER_GROWTH_LO) * s_rows
                    )
                    residual_so = _clip_like_reference_per_row_growth(
                        residual_so,
                        residual_prev,
                        growth_row,
                    )
                    v_residual = v_group + residual_so
                    cos_mix = _velocity_mix_alpha(v_residual, v_group)
                    alpha_floor = (
                        _MTA_RAB2_EXPLORER_ALPHA_FLOOR_LO
                        + (
                            _MTA_RAB2_EXPLORER_ALPHA_FLOOR_HI
                            - _MTA_RAB2_EXPLORER_ALPHA_FLOOR_LO
                        )
                        * s_rows
                    )
                    alpha = alpha_floor + (1.0 - alpha_floor) * cos_mix
                    alpha = alpha.view(-1, *([1] * (residual_so.ndim - 1)))
                    v_hat = v_group + alpha * residual_so
                else:
                    if i >= 2:
                        v_so = _MTA_RAB2_C1 * v_prev + _MTA_RAB2_C2 * v_pp
                    else:
                        v_so = v_prev
                    v_hat = v_so
            ...
            prev_prev_group_velocities.copy_(old_group)
            if infer_mask.any():
            ...
            if infer_mask.any():
                prev_group_velocities.copy_(v_mean)
            else:
                prev_group_velocities.copy_(old_group)
...
# Alias for comparison scripts (``test_qwen_rollout_comparison``).
sample_reference_model_rab2_explorer = sample_reference_model
...
def _apply_rab2_preset(preset: str) -> None:
    """Set rollout knobs for training."""
    global _MTA_RAB2_EARLY_FRAC, _MTA_RAB2_C1, _MTA_RAB2_C2
    global _MTA_RAB2_RESIDUAL_BIAS, _MTA_RAB2_RESIDUAL_GROWTH
    if preset == "default":
        _MTA_RAB2_EARLY_FRAC = 0.4
        _MTA_RAB2_C1 = 2.0
        _MTA_RAB2_C2 = -1.0
...
def main(args):
    ...
    _apply_rab2_preset(args.rab2_preset)
    ...
    if rank == 0:
        ...
        main_print(
            f"RAB2-Explorer preset={args.rab2_preset}: early_frac={_MTA_RAB2_EARLY_FRAC}, "
            f"c1={_MTA_RAB2_C1}, c2={_MTA_RAB2_C2}; early fusion uses per-rollout "
            f"alpha_floor in [{_MTA_RAB2_EXPLORER_ALPHA_FLOOR_LO}, {_MTA_RAB2_EXPLORER_ALPHA_FLOOR_HI}], "
            f"growth in [{_MTA_RAB2_EXPLORER_GROWTH_LO}, {_MTA_RAB2_EXPLORER_GROWTH_HI}] "
            f"(rab2_preset bias/growth apply only to non-explorer code paths); "
            f"zero_loss_on_guess={getattr(args, 'zero_loss_on_guess', False)}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rollout_image_dir",
        type=str,
        default="data/outputs/rollout_scratch_train_rab2_explorer",
        help="Temporary decoded PNG directory during rollout for RAB2-Explorer.",
    )

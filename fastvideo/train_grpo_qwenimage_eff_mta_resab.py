...
def run_sample_step(
    ...
    if grpo_sample:
        ...
        for i in progress_bar:
            ...
            # MTA-ResAB：v_tgt 为 (x_L_mean-z)/dσ；前半段 v_hat = v_tgt + η*(v_so-v_prev)，η 由 cos 标量；后半段仅 AB2
            early_group_fusion = i < len(progress_bar) * 0.5
            if guessed_mask.any():
                ds = d_sigma if abs(d_sigma) > 1e-6 else 1e-6
                v_tgt = (x_L_mean[guessed_mask] - z[guessed_mask].to(torch.float32)) / ds
                ...
                if early_group_fusion:
                    d_acc = v_so - v_prev
                    eta = _velocity_mix_alpha(d_acc, v_tgt - v_prev)
                    eta = eta.view(-1, *([1] * (v_prev.ndim - 1)))
                    v_hat = v_tgt + eta * d_acc
                else:
                    v_hat = v_so

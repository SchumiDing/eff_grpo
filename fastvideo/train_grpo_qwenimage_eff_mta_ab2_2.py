...
# MTA-AB2-2: 与 AB2 同前半窗；Drawbench 上 p>1 略损 HPS，改用 p<1 极弱抬高 v_so
_MTA_AB2_2_EARLY_FRAC = 0.50
_MTA_AB2_2_ALPHA_POW = 0.99
...
def run_sample_step(
    ...
    if grpo_sample:
        ...
        for i in progress_bar:
            ...
            # 预测分支：AB2-2 — early 同 AB2；a=alpha^p，p<1 时略增 v_so
            early_group_fusion = i < len(progress_bar) * _MTA_AB2_2_EARLY_FRAC
            if guessed_mask.any():
                ...
                if early_group_fusion:
                    ...
                    a = alpha.clamp(0.0, 1.0).pow(_MTA_AB2_2_ALPHA_POW)
                    v_hat = a * v_so + (1.0 - a) * v_group
                else:
                    v_hat = v_so

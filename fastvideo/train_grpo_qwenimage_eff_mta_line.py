...
def run_sample_step(
    ...
    if grpo_sample:
        ...
        for i in progress_bar:
            ...
            # MTA-Line：v_tgt=(x_L_mean-z)/dσ；前半段将 (v_prev-v_pp) 按 cos 加权叠到 v_tgt；后半段纯 AB2
            ...
            if guessed_mask.any():
                ...
                if early_group_fusion:
                    if i >= 2:
                        dv = v_prev - v_pp
                        w = _velocity_mix_alpha(dv, v_tgt - v_prev)
                        w = w.view(-1, *([1] * (v_prev.ndim - 1)))
                        v_hat = v_tgt + w * dv
                    else:
                        v_hat = v_tgt
                else:
                    if i >= 2:
                        v_hat = 1.5 * v_prev - 0.5 * v_pp
                    else:
                        v_hat = v_prev

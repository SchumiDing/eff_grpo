...
#
# Residual-AB2 rollout fork: predict rollout-specific residual velocity around the
# current per-prompt group mean, then re-anchor to the fresh group estimate.
# The goal is to preserve per-rollout diversity while keeping global guidance stable.
        ...
        for i in progress_bar:
            ...
            if guessed_mask.any():
                ...
                if early_group_fusion:
                    ...
                    residual_so = _clip_like_reference(
                        residual_so,
                        residual_prev,
                        _MTA_RAB2_RESIDUAL_GROWTH,
                    )
                    v_residual = v_group + residual_so
                    alpha = _velocity_mix_alpha(v_residual, v_group)
                    alpha = _MTA_RAB2_RESIDUAL_BIAS + (1.0 - _MTA_RAB2_RESIDUAL_BIAS) * alpha
...
def _rab2_wrap(early_frac: float, c1: float, c2: float, residual_bias: float, residual_growth: float):
    """Return a sample_reference_model-compatible callable with temporary RAB2 knobs."""
    ...
    def wrapped(
        ...
        old = (
            _MTA_RAB2_EARLY_FRAC,
            _MTA_RAB2_C1,
            _MTA_RAB2_C2,
            _MTA_RAB2_RESIDUAL_BIAS,
            _MTA_RAB2_RESIDUAL_GROWTH,
        )
        _MTA_RAB2_EARLY_FRAC = early_frac
        _MTA_RAB2_C1 = c1
        _MTA_RAB2_C2 = c2
        _MTA_RAB2_RESIDUAL_BIAS = residual_bias
        _MTA_RAB2_RESIDUAL_GROWTH = residual_growth
        try:
...
# Residual-AB2 presets (names match test_qwen_rollout_comparison method keys).
sample_reference_model_rab2 = _rab2_wrap(0.4, 2.0, -1.0, 0.25, 1.35)
...
def main(args):
    ...
    if rank == 0:
        ...
        main_print(
            f"RAB2 preset={args.rab2_preset}: early_frac={_MTA_RAB2_EARLY_FRAC}, "
            f"c1={_MTA_RAB2_C1}, c2={_MTA_RAB2_C2}, residual_bias={_MTA_RAB2_RESIDUAL_BIAS}, "
            f"residual_growth={_MTA_RAB2_RESIDUAL_GROWTH}; zero_loss_on_guess={getattr(args, 'zero_loss_on_guess', False)}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rollout_image_dir",
        type=str,
        default="data/outputs/rollout_scratch_train_rab2",
        help="Temporary decoded PNG directory during rollout for residual-AB2.",
    )

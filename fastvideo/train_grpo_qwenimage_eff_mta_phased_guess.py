...
#
# Phased guess rollout: for the first guess_phase_train_steps training steps,
# each prompt uses num_infer + num_guess rollouts with up to num_guess mock-guesses
# per denoising step (same MTA/AB2 logic as ab2). After that, only num_infer
# rollouts and no guessing (full infer every step).
...
def phased_rollout_sizes(args, train_step: int) -> Tuple[int, int]:
    """Return (num_generations, num_guess_per_step) for this training step."""
    x = args.num_infer
    y = args.num_guess
    cap = getattr(args, "guess_phase_train_steps", None)
    if cap is None or train_step < cap:
        return x + y, y
    return x, 0
...
def run_sample_step(
        ...
        txt_seq_lens,
        grpo_sample,
        rollout_num_generations: Optional[int] = None,
        rollout_num_guess: Optional[int] = None,
    ):
    if grpo_sample:
        all_latents = [z]
        all_log_probs = []
        ...
        B = z.shape[0]
        num_generations = (
            rollout_num_generations
            if rollout_num_generations is not None
            else args.num_generations
        )
        num_guess = (
            rollout_num_guess if rollout_num_guess is not None else args.num_guess
        )
        ...
        for i in progress_bar:
            ...
            per_prompt_num_guess = 0 if i == args.sampling_steps - 1 else num_guess
...
def sample_reference_model(
    ...
    with torch.no_grad():
        z, latents, all_latents_tensor, all_log_probs_tensor, all_mock_flags_tensor = run_sample_step(
            ...
            txt_seq_lens,
            grpo_sample,
            rollout_num_generations=rollout_num_generations,
            rollout_num_guess=rollout_num_guess,
        )
...
def train_one_step(
    ...
    eff_ng, eff_nguess = phased_rollout_sizes(args, train_step)
    ...
    if args.use_group:
        def repeat_tensor(tensor):
            if tensor is None:
                return None
            return torch.repeat_interleave(tensor, eff_ng, dim=0)
        ...
        if isinstance(caption, str):
            caption = [caption] * eff_ng
        elif isinstance(caption, list):
            caption = [item for item in caption for _ in range(eff_ng)]
        else:
    ...
    reward, all_latents, all_log_probs, sigma_schedule, all_txt_seq_lens, all_mock_flags = sample_reference_model(
            ...
            caption,
            preprocess_val,
            rollout_num_generations=eff_ng,
            rollout_num_guess=eff_nguess,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,   
        help="Fallback when rollout overrides are not used (e.g. tests). Phased training uses num_infer+num_guess / num_infer.",
    )
    parser.add_argument(
        "--guess_phase_train_steps",
        type=int,
        default=None,
        help=(
            "For training steps in [0, guess_phase_train_steps), use num_infer+num_guess rollouts per prompt "
            "with up to num_guess mock-guesses per denoising step. After that, use num_infer rollouts only (no guess). "
            "If None, always stay in the guess phase (same as infinite warmup)."
        ),
    )
    parser.add_argument(
        "--use_hpsv2",
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="x: infer rollouts per prompt; after guess phase, total rollouts per prompt = x only.",
    )
    parser.add_argument(
        "--num_guess",
        type=int,
        default=4,
        help="y: during guess phase, max mock-guesses per denoising step per prompt; total rollouts then = num_infer + num_guess.",
    )

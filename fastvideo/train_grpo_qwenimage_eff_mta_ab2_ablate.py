...
#
# AB2 hyperparameter ablation fork: same logic as train_grpo_qwenimage_eff_mta_ab2.py but
# run_sample_step reads _MTA_AB2_ABL_* knobs. Wrappers below set them per-method for eval.
# The canonical AB2 file is intentionally unchanged.
...
# Canonical AB2: early_frac=0.5, v_so = 1.5*v_prev - 0.5*v_pp  => c1=1.5, c2=-0.5
...
def run_sample_step(
    ...
    if grpo_sample:
        ...
        for i in progress_bar:
            ...
            early_group_fusion = i < len(progress_bar) * _MTA_AB2_ABL_EARLY_FRAC
            if guessed_mask.any():
                v_prev = old_prev[guessed_mask]
                v_pp = old_pprev[guessed_mask]
                if i >= 2:
                    v_so = _MTA_AB2_ABL_C1 * v_prev + _MTA_AB2_ABL_C2 * v_pp
                else:
                    v_so = v_prev
...
def _ablate_wrap(early_frac: float, c1: float, c2: float):
    """Return a sample_reference_model-compatible callable with temporary AB2 knobs."""
    ...
    def wrapped(
        ...
        old = (_MTA_AB2_ABL_EARLY_FRAC, _MTA_AB2_ABL_C1, _MTA_AB2_ABL_C2)
        _MTA_AB2_ABL_EARLY_FRAC = early_frac
        _MTA_AB2_ABL_C1 = c1
        _MTA_AB2_ABL_C2 = c2
        try:
            ...
            _MTA_AB2_ABL_EARLY_FRAC, _MTA_AB2_ABL_C1, _MTA_AB2_ABL_C2 = old

    return wrapped


# Ablation grid (names match test_qwen_rollout_comparison method keys):
# - early_frac: fraction of steps with v_group fusion
# - c1,c2: v_so = c1*v_prev + c2*v_pp for i>=2 (canonical AB2: 1.5, -0.5)
sample_reference_model_ab2_abl_ef04 = _ablate_wrap(0.4, 1.5, -0.5)
sample_reference_model_ab2_abl_ef06 = _ablate_wrap(0.6, 1.5, -0.5)
sample_reference_model_ab2_abl_v1 = _ablate_wrap(0.5, 1.0, 0.0)  # first-order: v_so = v_prev
sample_reference_model_ab2_abl_strong = _ablate_wrap(0.5, 2.0, -1.0)  # stronger linear extrapolation
# 融合：early 0.4（更短 v_group 融合窗）+ 强外推 2*v_prev - v_pp
sample_reference_model_ab2_abl_ef04_strong = _ablate_wrap(0.4, 2.0, -1.0)
